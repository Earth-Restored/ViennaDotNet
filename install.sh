...
if [ "$OS" = "Darwin" ]; then
    ...
    if [ "${EUID:-$(id -u)}" -eq 0 ]; then
        ...
        [ -n "$CURRENT_USER" ] || err "Run this script via sudo from the account that should own the installation."
    else
        CURRENT_USER=$(id -un)
    fi
    HOME_DIR=$(getent passwd "$CURRENT_USER" | cut -d: -f6)
fi
[ -n "$HOME_DIR" ] || HOME_DIR=$(eval echo "~$CURRENT_USER")
...
stop_service
killall -9 dotnet java pwsh 2>/dev/null || true
sleep 2
